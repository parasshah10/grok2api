...
class FileStorage(BaseStorage):
    ...
    async def init_db(self) -> None:
        ...
        if not self.token_file.exists():
            ...
            logger.info("[Storage] Created new token file")
        ...
        if not self.config_file.exists():
            ...
            logger.info("[Storage] Created new config file")
...
class MysqlStorage(BaseStorage):
    ...
    async def init_db(self) -> None:
        ...
        try:
            ...
            logger.info(f"[Storage] Parsing DB connection: {parsed['user']}@{parsed['host']}:{parsed['port']}/{parsed['db']}")
            ...
            # Initialize file storage and sync data
        ...
        try:
            async with temp_pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    logger.info(f"[Storage] MySQL database '{parsed['db']}' ready")
        finally:
            temp_pool.close()
    ...
    async def _create_tables(self) -> None:
        ...
        async with self._pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                logger.info("[Storage] MySQL tables created/verified")
        ...
        try:
            for table, key in [("grok_tokens", "sso"), ("grok_settings", "global")]:
                ...
                if data:
                    ...
                    logger.info(f"[Storage] {table.split('_')[1]} data synced from DB to file")
                else:
                    if table == "grok_tokens":
                        file_data = await self._file.load_tokens()
                        if file_data.get(key) or file_data.get("ssoSuper"):
                            await self._save_db(table, file_data)
                            logger.info("[Storage] Token data initialized from file to DB")
                    else:
                        file_data = await self._file.load_config()
                        if file_data.get(key) or file_data.get("grok"):
                            await self._save_db(table, file_data)
                            logger.info("[Storage] Config data initialized from file to DB")
        except Exception as e:
            logger.warning(f"[Storage] Data sync failed: {e}")
    ...
    async def _load_db(self, table: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[Storage] Failed to load {table} from DB: {e}")
            return None
    ...
    async def close(self) -> None:
        ...
        if self._pool:
            ...
            logger.info("[Storage] MySQL pool closed")
        ...
        try:
            for key, file_func, key_name in [
                ...
                if data:
                    ...
                    logger.info(f"[Storage] {key.split(':')[1]} data synced from Redis to file")
                else:
...
class StorageManager:
    ...
    async def init(self) -> None:
        ...
        logger.info("[Storage] Storage Manager initialized")
