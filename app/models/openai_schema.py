...
class OpenAIChatRequest(BaseModel):
    ...
    messages: List[Dict[str, Any]] = Field(..., description="List of messages", min_length=1)
    stream: bool = Field(False, description="Enable stream response")
    ...
    def validate_messages(cls, v):
        ...
        for msg in v:
            ...
            if 'role' not in msg:
                raise HTTPException(
                    status_code=400,
                    detail="Message missing required field 'role'"
                )
            if 'content' not in msg:
                raise HTTPException(
                    status_code=400,
                    detail="Message missing required field 'content'"
                )
...
class OpenAIChatCompletionChoice(BaseModel):
    ...
    logprobs: Optional[float] = Field(default=None, description="Logprobs")
